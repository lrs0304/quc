...
ColorWidget *ColorWidget::self = 0;
ColorWidget::ColorWidget(QWidget *parent) : QWidget(parent)
{
    ...
    labColor->setFont(QFont("Arial", 30, QFont::Bold));
    ...
    if (!pixmap.isNull()) {
        ...
        if (!image.isNull()) {
            if (image.valid(0, 0)) {
                ...
                green = color.green();
                blue = color.blue();
                QString strRed = tr("%1").arg(red & 0xFF, 2, 16, QLatin1Char('0'));
                QString strGreen = tr("%1").arg(green & 0xFF, 2, 16, QLatin1Char('0'));
                QString strBlue = tr("%1").arg(blue & 0xFF, 2, 16, QLatin1Char('0'));
